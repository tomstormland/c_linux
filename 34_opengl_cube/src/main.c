...
int main(int argc, char *argv[]) {
  ...
  unsigned char *data = stbi_load("texture.jpg", &width, &height, &nr_channels, 0);
  if (!data) {
    printf("ERROR: 'Unable to load texture texure.jpg.'.\n");
    return -1;
  }
  ...
  while (is_open) {
    ...
    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
    mat4x4_identity(model_matrix);
    mat4x4_rotate(model_matrix, model_matrix, 1.0f, 1.0f, 1.0f, angle);
    glUniformMatrix4fv(model_uniform_location, 1, GL_FALSE, (const float*)model_matrix);
    glDrawArrays(GL_TRIANGLES, 0, 36);
    glXSwapBuffers(display, window);

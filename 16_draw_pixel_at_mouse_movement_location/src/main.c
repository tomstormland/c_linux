...
int interface_initialize(struct Interface *self) {
  ...
  XSelectInput(self->display, self->window, ExposureMask|KeyPressMask|ButtonPressMask|ButtonReleaseMask|PointerMotionMask);
...
int interface_main_loop(struct Interface *self) {
  ...
  while (self->open) {
    ...
    if (self->xevent.type == MotionNotify) {
      int x = self->xevent.xmotion.x;
      int y = self->xevent.xmotion.y;
